...
export class CreateFormTopics1763165894976 implements MigrationInterface {
    name = 'CreateFormTopics1763165894976'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`form_topics\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuid\` varchar(255) NOT NULL, \`created_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updated_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, \`form_id\` int NOT NULL, \`title\` varchar(255) NOT NULL, \`description\` text NULL, \`order\` int NOT NULL, \`formId\` int NULL, INDEX \`IDX_823deb82c9bf668099524cba11\` (\`form_id\`), UNIQUE INDEX \`IDX_5707656053d4ff1d94bafd4b2a\` (\`uuid\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`form_topics\` ADD CONSTRAINT \`FK_7eb3eccdbad92eaf6cd90796abc\` FOREIGN KEY (\`formId\`) REFERENCES \`forms\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`form_topics\` DROP FOREIGN KEY \`FK_7eb3eccdbad92eaf6cd90796abc\``);
